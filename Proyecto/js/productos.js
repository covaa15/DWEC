...
function mostrarProductos(productos) {
    ...
    productos.forEach(producto => {
        ...
        <p class="precio"><strong>$${convertirPrecio(producto.precio)} ${monedaActual}</strong></p>
...
function cargarProductoEnModal(producto) {
    ...
    document.getElementById("modalPrecio").textContent = convertirPrecio(producto.precio) + " " + monedaActual;
...
function actualizarVentanaCarrito() {
    ...
    if (carrito.length === 0) {
        ...
        totalTxt.textContent = `Total pagar: $0 ${monedaActual}`;
        return;
    }
    ...
    carrito.forEach(item => {
        total += item.precio * item.cantidad * (tasasConversion[monedaActual] || 1);

        const tr = document.createElement("tr");
        tr.innerHTML = `
            <td><img src="${item.imagen}" class="imgCarrito"></td>
            <td>${item.nombre}</td>
            <td>$${convertirPrecio(item.precio)}</td>
            <td>
    ...
    totalTxt.textContent = `Total pagar: $${total.toFixed(2)} ${monedaActual}`;
...
Cuandoen el carrito se reduce a 0 la cantidad este se va a eliminar 
del carrito*/
...
/*Cuando pulso el boton de Realizar Compra, primero compruebo que existan productos
en el carrito, en el caso de que no existan muestro un mensaje indicando que no hay productos
y en el caso de que existan muestro un mensaje de confirmacion de la compra */
...
async function cargarTasas() {
    try {
        const response = await fetch("https://v6.exchangerate-api.com/v6/8e0fb0a2bcf626875e04d545/latest/USD");
        const data = await response.json();
        if (data.result !== "success") throw new Error("No se pudo cargar las tasas de cambio");
        tasasConversion = data.conversion_rates;
...
function actualizarPreciosProductos() {
    document.querySelectorAll(".card .precio").forEach((precioElemento) => {
        ...
        if (producto) {
            precioElemento.textContent = `$${convertirPrecio(producto.precio)} ${monedaActual}`;
        }
    });
