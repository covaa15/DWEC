...
function aplicarFiltros() {
    ...
    inputs.forEach(unInput=>{
        unInput.addEventListener('change',function(){
            ...
            const fechaSeleccionada=unInput.value;
            const artistaFiltrados=artistas.filter(artista=>{
                const horario=obtenerHorario(artista.id);
                return horario.dia===fechaSeleccionada;
            });
            mostrarArtistas(artistaFiltrados);
        })
    })
...
function activarBotones(){
    ...
    document.querySelectorAll("button").forEach(btn => {
        btn.addEventListener("click", () => {
            if(!planesAñadidos.includes(btn.id)){
                planesAñadidos.push(btn.id);
                ...
                const botonReseña=document.createElement('button');
                botonReseña.textContent="Reseña Enviada";
                botonReseña.id=artista.id;
                botonReseña.className="reseña";
    
                const botonEliminar=document.createElement('button');
                botonEliminar.textContent="Eliminar";
                botonEliminar.id=artista.id;
                botonEliminar.className="eliminar";
    
                div2.appendChild(botonReseña);
                ...
                window.alert("El plan ya esta añadido");
            }
    ...
    document.querySelectorAll(".eliminar").forEach(btn => {
        btn.addEventListener("click", () => {
            const divEliminar=document.getElementById(btn.id);
            listaPlanes.remove(divEliminar);
...
function obtenerHorarioFecha(fecha){
    let artista;
    horarios.forEach(horario => {

        if (horario.fecha === fecha)
            artista = horario.artistaId;
